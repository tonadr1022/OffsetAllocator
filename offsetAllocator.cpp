...
Allocation Allocator::allocate(uint32 size) {
  ...
  if (leafBinIndex == Allocation::NO_SPACE) {
    ...
    // All leaf bins here fit the alloc, since the top bin was rounded up. Start leaf search from
    // bit 0. NOTE: This search can't fail since at least one leaf bit was set because the top bit
    // was set.
...
bool Allocator::grow(uint32 extraBytes, uint32 extraNodeCapacity) {
  if (extraBytes == 0 && extraNodeCapacity == 0) return true;

  if (extraNodeCapacity == 0) {
    extraNodeCapacity = 128 * 1024;
  }

  {
    const uint32 oldMax = m_maxAllocs;
    const uint32 newMax = m_maxAllocs + extraNodeCapacity;
    auto* newNodes = new Node[newMax];
    auto* newFreeNodes = new NodeIndex[newMax];
    if (!newNodes || !newFreeNodes) {
      delete[] newNodes;
      delete[] newFreeNodes;
      return false;
    }

    if (m_nodes) std::memcpy(newNodes, m_nodes, sizeof(Node) * oldMax);
    if (m_freeNodes) std::memcpy(newFreeNodes, m_freeNodes, sizeof(NodeIndex) * (m_freeOffset + 1));

    delete[] m_nodes;
    delete[] m_freeNodes;
    m_nodes = newNodes;
    m_freeNodes = newFreeNodes;
    m_maxAllocs = newMax;

    for (uint32 idx = oldMax; idx < newMax; ++idx) {
      m_freeNodes[++m_freeOffset] = (NodeIndex)idx;
    }
  }
  ...
  NodeIndex tailFreeIdx = Node::unused;
  NodeIndex tailPrevIdx = Node::unused;

  for (uint32 i = 0; i < m_maxAllocs; ++i) {
    if (i == Node::unused) continue;
    const Node& n = m_nodes[i];
    if (!n.used && (n.dataOffset + n.dataSize == oldSize)) {
      tailFreeIdx = (NodeIndex)i;
      tailPrevIdx = n.neighborPrev;
      break;
    }
  }

  if (tailFreeIdx != Node::unused) {
    const Node& tail = m_nodes[tailFreeIdx];

    const NodeIndex neighborPrev = tail.neighborPrev;
    const NodeIndex neighborNext = tail.neighborNext;
    assert(neighborNext == Node::unused);

    removeNodeFromBin(tailFreeIdx);

    const uint32 combinedSize = tail.dataSize + extraBytes;
    const uint32 combinedOff = tail.dataOffset;
    const NodeIndex combinedIdx = insertNodeIntoBin(combinedSize, combinedOff);

    if (neighborPrev != Node::unused) {
      m_nodes[combinedIdx].neighborPrev = neighborPrev;
      m_nodes[neighborPrev].neighborNext = combinedIdx;
    }
    if (neighborNext != Node::unused) {
      m_nodes[combinedIdx].neighborNext = neighborNext;
      m_nodes[neighborNext].neighborPrev = combinedIdx;
    }
  } else {
    const uint32_t newTailOffset = oldSize;
    const NodeIndex newIdx = insertNodeIntoBin(extraBytes, newTailOffset);

    NodeIndex prevIdx = Node::unused;
    for (uint32 i = 0; i < m_maxAllocs; ++i) {
      const Node& n = m_nodes[i];
      if (n.dataOffset + n.dataSize == oldSize) {
        prevIdx = (NodeIndex)i;
        break;
      }
    }
    if (prevIdx != Node::unused) {
      m_nodes[newIdx].neighborPrev = prevIdx;
      m_nodes[prevIdx].neighborNext = newIdx;
    }
  }
