...
TEST_CASE("numbers", "[SmallFloat]") {
  SECTION("uintToFloat") {
    ...
    NumberFloatUpDown testData[] = {
        {.number = 17, .up = 17, .down = 16},       {.number = 118, .up = 39, .down = 38},
        {.number = 1024, .up = 64, .down = 64},     {.number = 65536, .up = 112, .down = 112},
        {.number = 529445, .up = 137, .down = 136}, {.number = 1048575, .up = 144, .down = 143},
    };
...
TEST_CASE("allocate", "[offsetAllocator]") {
  ...
  SECTION("reuse trivial") {
    // Allocator should reuse node freed by A since the allocation C fits in the same bin (using
    // pow2 size to be sure)
  ...
  SECTION("reuse complex") {
    // Allocator should not reuse node freed by A since the allocation C doesn't fits in the same
    // bin However node D and E fit there and should reuse node from A
    OffsetAllocator::Allocation a = allocator.allocate(1024);
    REQUIRE(a.offset == 0);
  ...
  SECTION("zero fragmentation") {
    ...
    for (uint i = 0; i < 256; i++) {
      if (i < 152 || i > 154) allocator.free(allocations[i]);
    }
